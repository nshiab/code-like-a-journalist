...
To allow your code to connect to the internet, you need to pass the `--allow-net` flag in the terminal command. You can limit it to the host you need with `--allow-net=raw.githubusercontent.com`.

So instead of running your script with `deno run --watch --check main.ts` as instructed at the beginning of this lesson, use `deno run --watch --check --allow-net=raw.githubusercontent.com main.ts`.

Since we'll also write data to our project folder later, grant that specific access too.

Click on your terminal and press `CTRL + C` to stop the script from running (this works on both Mac and PC). Then run: `deno run --watch --check --allow-net=raw.githubusercontent.com --allow-write=. main.ts`

Unlike `-A` (`--allow-all`), these scoped flags keep the rest of Deno's permission sandbox in place.
