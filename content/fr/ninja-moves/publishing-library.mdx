...
Avant de téléverser quoi que ce soit, exécutez `deno publish --dry-run`. Deno vérifiera les types de l’API publique et appliquera les validations de JSR sans publier. Corrigez toute erreur signalée avant de continuer.

Lorsque cette simulation réussit, exécutez `deno publish` !
...
Tant qu’à faire, nous pouvons aussi enchaîner quelques commandes pour nous assurer que notre code est bien formaté (`deno fmt --check`) et que les types sont corrects (`deno check`). L’option `--check` vérifie le formatage sans modifier les fichiers pendant la publication.
...
{
  "tasks": {
    "test-publish": "deno fmt --check && deno check index.ts && deno test && deno publish"
  },
  "name": "@nshiab/my-first-library",
  "version": "0.1.0",
  "license": "MIT",
...
Mais si vous avez beaucoup de librairies à mettre à jour, exécutez `deno update`. Par défaut, la commande respecte les plages de versions de `deno.json`. Utilisez `deno update --latest` uniquement si vous voulez volontairement dépasser ces plages, notamment pour passer à une nouvelle version majeure.
...
4. Lorsque je suis prêt à publier une nouvelle version (souvent après plusieurs pull requests), j’exécute localement l’une des tâches `patch`, `minor` ou `major`, définies dans le fichier [`deno.json`](https://github.com/nshiab/simple-data-analysis/blob/main/deno.json) du dépôt.

5. Ces tâches exécutent toutes les vérifications, puis appellent l’outil [`@nshiab/bump-version`](https://jsr.io/@nshiab/bump-version). Il met à jour `deno.json`, valide et pousse ce changement, crée un [tag Git](https://git-scm.com/book/en/v2/Git-Basics-Tagging), puis pousse le tag sur GitHub.

6. Lorsqu’un nouveau tag est poussé sur GitHub, deux workflows sont déclenchés : [un pour publier](https://github.com/nshiab/simple-data-analysis/blob/main/.github/workflows/jsr-publish.yml) la librairie sur JSR, et un autre pour [créer une release](https://github.com/nshiab/simple-data-analysis/blob/main/.github/workflows/release.yml) sur GitHub avec un résumé de tous les changements issus des pull requests approuvées.
...
Avec JSR, vous pouvez [retirer votre librairie de l’index](https://jsr.io/docs/private-registries), ce qui la rend introuvable via la recherche, même si elle reste accessible publiquement. JSR ne permet pas encore de publier des librairies privées.
...
Vous avez terminé le parcours principal du cours. Continuez à explorer les personnes et les ressources inspirantes dans les prochaines pages, puis utilisez ce que vous avez appris pour créer vos propres projets de données.
