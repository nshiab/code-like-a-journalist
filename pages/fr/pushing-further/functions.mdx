...
En encapsulant du code dans des fonctions, vous pouvez réutiliser ce même code encore et encore. Les fonctions aident également à diviser de longs scripts en morceaux modulaires, ce qui les rend plus faciles à gérer et à déboguer.
...
Notre fonction est assez basique. Ce serait plus intéressant si nous pouvions **passer** un `name` (nom) pour une salutation personnalisée, comme `"Hello, Nael!"`.
...
```
<Callout type="info" emoji="💡">
    À la ligne 1, la syntaxe `name: string` est appelée un **type explicite**. Cela signifie que `name` doit être une `string`. La plupart du temps, les types dans votre code sont inférés. Mais dans certains cas, vous devez spécifier explicitement les types souhaités. La définition des paramètres de fonction est l’un de ces cas.
</Callout>
...
En réalité, les calculs sont effectués et retournés, mais retournés dans le vide ! Vous devez stocker le résultat quelque part ou l’afficher directement.
...
Comme pour les paramètres, les valeurs retournées peuvent être de n’importe quel type : chaînes de caractères, nombres, booléens, objets ou même fonctions.
...
Puisque `main.ts` est notre point d'entrée principal pour le code, nous essayons de le garder simple. L’encombrer avec des déclarations de fonctions n’est pas optimal.
...
Dans notre cas, nous voulons l’utiliser dans `main.ts`. Pour ce faire, ajoutez la ligne suivante au début du fichier :  
`import convertDollarsToEuros from "./helpers/convertDollarsToEuros.ts";`.

IntelliSense reconnaît que la fonction est disponible. Si vous commencez à taper, il vous la suggérera. Sympa, non ? Appuyez sur `Enter` pour accepter la suggestion.

Si vous êtes sur un PC sous Windows, vous pourriez voir `\` au lieu de `/` dans le chemin du fichier.
...
```
<Callout type="info" emoji="💡">
    Lorsque vous importez un fichier, vous devez indiquer à votre ordinateur où le trouver avec un **chemin**. Ici, `./helpers/convertDollarsToEuros.ts` est un **chemin relatif** car il commence par `./`. Le `.` indique à l’ordinateur de commencer là où il se trouve. Dans ce cas, puisque nous exécutons `main.ts`, il commence dans le dossier où se trouve `main.ts`. Ensuite, `/helpers` indique à l’ordinateur d’aller dans le dossier `helpers`. Enfin, `/convertDollarsToEuros.ts` lui indique d’importer le contenu de ce fichier. Sur un PC sous Windows, le chemin utilise `\` au lieu de `/`.
</Callout>
...
The reason why JSDoc is so handy is because VS Code understands it. If you hover over the function in `main.ts` now, you'll see not only the types but also your documentation popping up!
...
To ensure that VS Code (and other JSDoc parsers) recognize the example as code, we use a **fenced code block** with three backticks `` ``` ``. When opening the block, we also specify `ts` so VS Code knows it's TypeScript code and applies the relevant syntax highlighting.
...
Dans l'exemple ci-dessus, il n'y a pas de bénéfice significatif à utiliser une fonction fléchée. Cependant, lorsque vous travaillez avec des méthodes de tableau, les fonctions fléchées peuvent simplifier et rendre votre code plus lisible.

Dans le code ci-dessous, nous avons un tableau de nombres que nous voulons filtrer, en gardant uniquement les nombres positifs. La méthode `.filter()` nécessite une fonction en tant que paramètre, nous déclarons donc la fonction `isPositive` et la passons à `.filter()`.
...
Passons maintenant à la pratique avec la prochaine leçon : le refactoring. Je sais, cela peut sembler intimidant, mais ne vous inquiétez pas—je suis avec vous ! 🕺
