...
Maintenant, pour exécuter et surveiller ce fichier, nous utilisions auparavant la commande `deno run --watch --check --allow-net=raw.githubusercontent.com main.ts`, mais c'est un peu fastidieux à taper, et retenir toutes ces options est fatigant. Plus vos projets deviennent complexes, plus ces commandes de terminal peuvent s'allonger.
...
{
  "tasks": {
    "main": "deno run --watch --check --allow-net=raw.githubusercontent.com main.ts"
  }
}
```
...
La tâche enregistre la permission réseau exacte du script. Vous n'avez donc pas besoin de vous en souvenir et le script ne peut toujours pas contacter d'autres hôtes. Exécutez `deno task` sans nom pour afficher les tâches disponibles.
