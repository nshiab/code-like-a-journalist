...
[GitHub](https://github.com/) is a platform owned by Microsoft. It became the place of choice for many developpers and companies working on open-source projects. You can host public projects for free on GitHub and enjoy the many great features that will make your coding life easier.

You can also create private repositories with a free account, of course. And [you can pay if you need more ressources](https://github.com/pricing). But I've been using GitHub for my open-source projects for years (the [SDA](https://github.com/nshiab/simple-data-analysis) and [journalism](https://github.com/nshiab/journalism) libraries, and [this course too](https://github.com/nshiab/code-like-a-journalist)) and I never reached the limits of the free plan!

As its name states it: GitHub works with Git. So I strongly suggest you follow the previous lesson [How to use Git?](/git-and-github/git-basics) before diving into this one. Here, we are going to follow the same steps as the previous lesson: building a simple program to retrieve the temperature in Montreal. But we are going to store our code in the cloud, with GitHub!

<NoticeIntro />

## GitHub account

To work with Git, you first need to [create an account](https://github.com/). So start with that.

![GitHub homepage.](/assets/git-and-github/github-basics/github-account.png)

## A social network

When you log to GitHub and land on your home page, you'll see a **Feed** section. That's because GitHub is also a social network. For example, you can see my feed below as the current time of writing. It's great to stay up-to-date with open-source projects you are interested in or just to see what people you find interesting are working on.

![GitHub feed.](/assets/git-and-github/github-basics/feed.png)

Users have a public profile showing their public activity and you can follow them to see their activity in your feed. [Here's my profil for example](https://github.com/nshiab). **Follow me!** 🤗

Personnally, I like to follow other computational journalists or maintainers of open-source projects I am using.

You can update your profile and settings by clicking on the button at top right corner.
...
You can also **Watch** a project to see its activity on your feed. I usually watch for the releases of new version of open-source projects I am using, just stay up-to-date.

![GitHub repository.](/assets/git-and-github/github-basics/sda-repo.png)

## Creating a repository

Let's create your first repository! Click on the + button on the top right corner.

![Creating a new repository.](/assets/git-and-github/github-basics/new-button.png)

Then give you repository a name and make it private (so you won't be afraid of making mistakes 😉). No need for a `readme` and `.gitignore` for now. We'll talk about it later on. Then hit `Create repository` at the bottom.
...
We are going to follow the steps on pushing `an existing repository from the command line`. Keep this page open.

![The steps to connect an existing repo to a new one.](/assets/git-and-github/github-basics/new-repo-steps.png)

Create a new folder with the same name as the repository and open it with VS Code. Add a `main.ts` in it with a simple log for now.
...
Right now, your code is saved with Git locally, on our computer. It's time to save it remotely, in the cloud, on GitHub!

You can copy the instructions on the GitHub page and paste them in your terminal. Then press enter to run them! You URL should be different than mine but the rest should be similar.
...
Note that you'll probably be asked to identify yourself. Follow the suggested steps. If you are asked about a password, **you won't see what you are typing for security reasons**, but typing actually works! It's always weird the first time. 🤪

![Syncing the local and remote repositories.](/assets/git-and-github/github-basics/sync.png)

And if you refresh your repository page, you'll see your code synced! Your `main.ts` file is there and you see your latest commit.

![The remote repository page.](/assets/git-and-github/github-basics/repo-page-sync.png)

## Pushing to the repository

There is no `README.md` in your repository and GitHub is (strongly) suggesting that we add one. This file content will be displayed on the repository page. It's usually the first thing people will read before checking the code. So let's do that.

As the `.md` extension suggests, it's a Mardown file. Markdown is a convenient and simple syntax often used for documentation. The code will be rendered into good looking text by the tool or platform you are using. You can focus on the content without worrying about the style, which is nice! We will write some markdown together, but if you want to know more, check this [cheat sheet](https://www.markdownguide.org/cheat-sheet/).
...
The `#` is used for titles. The `[text](link)` syntax allow you to add links to your text and the use of backticks creates code blocks.
...
Tada! 🪄 It's now on GitHub! And your README is used to welcome the visitors!
...
Your issue is now created! Each issue has a unique ID (here it's `#1`) and people (or youself 🥸) can comment on it.
...
GitHub will automatically generate a name for the branch and will suggest to branch out from `main` which is exactly what we want.
...
Let's add the code from the previous lesson into our `main.ts`. This script pings the Meteorological Service of Canada API and retrieve data as XML. The `fast-xml-parser` library converts it to JSON. And the rest of the code extracts the temperature and the time, before logging them.
...
Install the `fast-xml-parser` dependency by running deno `deno add npm:fast-xml-parser`.
...
GitHub will check that you can actually merge the two branches and that they are no conflicts between the two. You can also add automatically trigerred tests to check that core features or functions of your project are not getting broken by the changes.
...
Once the merge confirmed, the changes are brought to the `main` branch!
...
This all happened in the cloud, on GitHub platform. To bring the changes to your local repository, first checkout to the `main` branch:
- `git checkout main`
...
Now, let's imagine that your computer took fire and is completly destroyed. 🔥

How can you retrieve all of your precious work? By **cloning** the repository.

Delete the folder you worked on so far. Yes, delete it! Trust me!

On your repository page, click on the `<> Code` button. There is where you'll have options to clone your code.
...
The dot at the end is important. It tells git to clone the repo in the current folder. Without it, it will create a new folder in your folder.
...
Once a repository is cloned, the next step is usually to install all dependencies with `deno install`. Because we have a `deno.json` in our project, Deno will now exactly what to install and which versions.

And then you are good to go to keep working on your amazing project!

## Forking a repository

If everyone can clone a public repository (or a private one if they were invited to collaborate on it), not everyone is allowed to push to it and merge in it. And this is a good thing! Imagine the mess and the critical security problems! 😈
...
It works kind like of a *branch of a repository*. There is no impact on the original repository, but if you want, you can create a pull request aimed at the owners of the original repository.

For example, as the time of writing, 16 GitHub users forked the [Simple Data Analysis library](https://github.com/nshiab/simple-data-analysis). I have no idea what they are doing with it and that's okay!

If you are curious, feel free to fork it too!

![Forking SDA.](/assets/git-and-github/github-basics/forking.png)

Forking is how a lot of open sources contributions and collaborations happen:
- Someone creates an open-source project.
- Someone else finds it interesting but want to tweak or fix something. They fork it, update the code, and send a PR to the owner of the original repository.
- The owner checks the PR and if happy merges it.
- The contributions is now part of the project and everyone can benefit from it!

## Conclusion

Git and GitHub work amazingly well together. With Git, you can keep track of your work while GitHub adds a useful interface to help you organize your work and help you collaborate with others.
...
- `git clone` to copy a whole repository on your computer

You have also learned about GitHub issues and pull requests.
...
And that's totally fine. Just find the recipe that works for you. But keep in mind that creating issues and reviewing pull requests are a great way to organize yourself and review your own work! It's also an opportunity to be confortable with tools used by many teams and companies around the world.

And when we will use GitHub actions to automate tasks like testing, web scraping, publishing libraries and webpages, you'll be amazed by everything this workflow has to offer.

Interested for more? Join me for the next lesson about GitHub Actions! 🤩
