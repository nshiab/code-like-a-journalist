...
To retrieve it, you need to extract the data directly from the HTML code and, on more complex websites, you need to automate/simulate clicks on the page to get want you want.

A word of warning: before scraping, **always make sure that what you are doing is legal**. Is some cases, scraping and copying data is prohited. Also, **always respect the infrastructure**. Don't overflow the websites with your requests. Be mindful of the resources and the costs your web scraping can involve for the people and organisations hosting the websites.

Note that I am expecting you to have completed the previous sessions of this course. The **4. Web basics 🌐** section in particuliar is important. If don't know how a webpage is build, it will be very difficult for you to extract data from it.

<NoticeIntro />

## Setup

To setup, we can use [setup-sda](https://github.com/nshiab/setup-sda), like we did for most of this course, but with the option `--scrape` to install some extra and handy libraries.
...
If you inspect the one named *European population dynamics, years 1000–1500* in a web browser (right-click on the table and then *Inspect* on the opening menu), you can see the table HTML element. If you explore its code, you'll see the various elements building that table with the data in it.

![Inspection an HTML page with Chrome.](/assets/web-scraping/wikipedia-table.png)
...
Copy and paste the code below into `sda/main.ts` and run `deno task sda` in your temrinal to start running and watching it.
...
The data returned by `getHtmlTable` is an array of objects, which means you can easily use it with the [Simple Data Analysis library](https://github.com/nshiab/simple-data-analysis) to cache it and process it.
...
PS: Note that you can download the data directly as a CSV. But I was looking for a public website that won't change much over time, and this one matches this creteria!
...
await sdb.done();
```
![MPs expenses extracted and summarize with SDA.](/assets/web-scraping/MPS-summary.png)
...
For example, here's the [list of all currently sitting Canadian Members of Parliament](https://www.ourcommons.ca/Members/en/search?parliament). This list changes over time, so might not have exactly the same as I have, which is coday for the purpose of this lesson.
...
To retrieve all the URLs, we can use [Playwright](https://github.com/microsoft/playwright). It's an open-source project by Microsoft. It's been created to automate the testing of websites, but it's also a wonderful web scraping tool. Playwright allows you to take control of a web browser with code and to extract what you want from the pages your code is visiting.

Playwright is installed automatically when you install everything your need with `setup-sda`.

If we inspect the cards, we can see the `a` tag contaning the link to the personal MP page. There's the same `ce-mip-mp-tile` class for all the tags.
...
- We retrieve all elements with the `ce-mip-mp-tile` (line 9).
- Then we use the `evaluateAll` method, which allows you to run code in the browser, which is very handy when web scraping. Here, we use it to easily extract the `href` attributes to retrieve the urls. (line 10).
- We log the urls (line 12).
- Finally, we close everything related to `chromium`.
...
If you have access to an AI in your browser developper tools (like in Chrome) and if you are unsure of the code you should write, you can ask it directly. It's pretty handy. Of course, be careful on what you are asking: this data is sent over the internet (here, to Google) and it's not always right!

For example, in the screenshot below, I inspected on the name of the MP, then right cliked on it in the HTML code and clicked on `Ask AI`. In the chat below, I asked `How can I retrieve the text content with Playwright?`. And the code provided actually works!

![All personal page urls for MPs.](/assets/web-scraping/ask-AI.png)

In the code below, we extract the name, paryt, district, province or territory, and of course preferred language of the MP.
...
- A tracker to have an estimated remainging time.
...
Now, when you run the script, you have a better idea of how fast it's going and how long it should take. Also, if it crashes at any time for any reason, you don't have to rescrape everything. The data extracted preivously has been saved a json files!
...
And now you'll see your code in action! This can be useful when trying to debug a script that doesn't work as expected.
...
So websites are more complicated to scrape. Retrieving a list of URLs is not always possible. Sometimes, you need to click on multiple menus and buttons to actually get to the data you want.

For example, if we want to retrieve the declared expenses of candidates to Canadian federal general elections, we need to click on a multiple options.
...
And finally we have access to the data, but we still need to iterate over a menu to extract data about each candidates.

![Candidates data on Elections Canada website.](/assets/web-scraping/EC-candidates-data.png)

Luckily, this is fairly easy to do with Playwright `selectOption` and `click` methods!

The code below has the option `{ headless: false }`, multiple `await page.waitForTimeout(500)` and a `scrollIntoViewIfNeeded` so you can see the code interacting with the page.
...
Sometimes, instead of Scraping  the HTML code, you directly use the API feeding data to the page. For example, [Yahoo Finance](https://finance.yahoo.com/) website displays a lot of data that could be scraped with Playwright. But another technic is to look for the API the page calls for data.
...
Note that this example comes from the [Stock market simulator 📈](/web-scraping) project. Check it if you want more about using undocumented APIs in your projects.
