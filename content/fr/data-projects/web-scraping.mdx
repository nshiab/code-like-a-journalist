...
Avant de commencer, terminez **Fondamentaux de TypeScript**, **Travailler avec TypeScript** et **Analyse de données avec SDA**. Nous aborderons les structures HTML nécessaires à l’extraction de données au fur et à mesure.

<NoticeIntro lang="fr" />

## Configuration

Pour configurer notre projet, utilisons [setup-data-project](https://github.com/nshiab/setup-data-project), comme dans les leçons précédentes.

Créez un nouveau dossier, ouvrez-le avec VS Code et exécutez `deno run -A jsr:@nshiab/setup-data-project`.

Lorsque le choix des librairies s'affiche, utilisez la barre d'espace pour sélectionner `simple-data-analysis`, `journalism-web-scraping` et `journalism-extras`, puis appuyez sur `Entrée`.

Nous utiliserons aussi Playwright et une fonction de la librairie de fichiers de Deno plus tard dans la leçon. Installez-les en exécutant `deno add npm:playwright-chromium jsr:@std/fs`. Autorisez ensuite le script d'installation de Playwright à télécharger Chromium en exécutant `deno install --allow-scripts=npm:playwright-chromium --node-modules-dir=auto`.
...
Par exemple, sur [cette révision figée d'une page Wikipédia](https://en.wikipedia.org/w/index.php?title=Medieval_demography&oldid=1372349632#Demographic_tables_of_Europe's_population) sur la démographie médiévale en Europe, vous pouvez voir plusieurs tableaux. La révision figée rend la leçon reproductible même lorsque Wikipédia change.
...
Comme ces structures HTML sont toujours les mêmes, j’ai publié une fonction pour extraire les données intégrées dans des tableaux comme celui-ci dans la librairie [journalism-web-scraping](https://github.com/nshiab/journalism-web-scraping), sélectionnée pendant la configuration.

### Avec un `index`

Vous pouvez passer à `getHtmlTable` l’URL que vous souhaitez utiliser. Ici, nous ciblons les tableaux ayant la classe CSS `.wikitable` et utilisons `index: 0` pour récupérer le premier résultat. Cette approche est plus fiable que de compter tous les tableaux de mise en page.
...
import { getHtmlTable } from "@nshiab/journalism-web-scraping";

const sdb = new SimpleDB();

const medievalData = await getHtmlTable(
  "https://en.wikipedia.org/w/index.php?title=Medieval_demography&oldid=1372349632",
  { selector: ".wikitable", index: 0 },
);
...
await medievalDemography.cache(async (table) => {
  const medievalData = await getHtmlTable(
    "https://en.wikipedia.org/w/index.php?title=Medieval_demography&oldid=1372349632",
    { selector: ".wikitable", index: 0 },
  );
  table.loadArray(medievalData);
});

medievalDemography.convert({
  Year: "number",
  "Total European population,millions": "number",
});
await medievalDemography.log();
...
<Callout type="info" emoji="💡">
    Mettre les données en cache est très important. Si vous ne vous attendez pas à ce que les données changent, vous pouvez les sauvegarder sur votre ordinateur au lieu de les récupérer encore et encore. Avec la librairie SDA, le cache est très simple à gérer. La méthode `cache` crée un dossier `.sda-cache` qui stocke les données dans votre projet. Si vous souhaitez que les données expirent après un certain temps, consultez l’option `ttl` dans la [documentation](https://jsr.io/@nshiab/simple-data-analysis-core/doc/~/SimpleTable.prototype.cache). Pour en savoir plus sur la librairie SDA, consultez la section [`3. Analyse de données avec SDA 🤓`](/simple-data-analysis/tabular-data).
</Callout>
...
await MPs.cache(async (table) => {
  const expensesData = await getHtmlTable(
    "https://www.ourcommons.ca/proactivedisclosure/en/members/2024/1",
    { selector: "#data-table" },
  );
  table.loadArray(expensesData);
});

MPs.replace(["Salaries", "Travel", "Hospitality", "Contracts"], {
  "$": "",
  ",": "",
});
MPs.convert({
...
MPs.summarize({
  columns: ["Salaries", "Travel", "Hospitality", "Contracts"],
  by: "Caucus",
  stats: "mean",
  decimals: 0,
});
MPs.wider("Caucus", "mean");
await MPs.log();
...
Par exemple, voici la [liste de tous les députés canadiens actuellement en fonction](https://www.ourcommons.ca/Members/en/search?parliament). Cette liste change volontairement avec le temps, donc vos résultats peuvent différer des captures d'écran. La page et les sélecteurs ci-dessous ont été vérifiés le 1er septembre 2026.
...
Nous avons installé Playwright pendant la configuration.

Si on inspecte les cartes sur le site web, on peut voir que la balise `a` contient le lien vers la page personnelle du député. Tous ces liens ont la classe `ce-mip-mp-tile-link`.
...
- On récupère tous les éléments avec la classe `ce-mip-mp-tile-link` (ligne 9).
- Puis on utilise la méthode `evaluateAll`, qui permet d’exécuter du code dans le navigateur. C’est très pratique pour le scraping. Ici, on l’utilise pour extraire facilement les attributs `href` et récupérer les URLs (ligne 10).
...
const urls = await page.locator(".ce-mip-mp-tile-link")
...
for (const url of urls) {
  ...
  const details = await page.locator("dl > dd").allTextContents();
  const district = details[1];
  const province = details[2];
  const language = details[3];
...
import { DurationTracker } from "@nshiab/journalism-extras";
...
const MPs = sdb.newTable("MPs");
MPs.loadData("sda/data/*.json");
MPs.summarize({
  by: ["party", "language"],
});
MPs.wider("party", "count");
await MPs.log();
...
Disons que nous voulons récupérer les dépenses déclarées des candidats à la 44e élection générale fédérale canadienne. Il faut cliquer sur plusieurs options sur la [page d’accueil](https://www.elections.ca/WPAPPS/WPF/EN/Home/Index). Les exemples utilisent la valeur d'évènement `53`, qui désigne cette élection terminée, et toute la séquence de sélecteurs a été vérifiée le 1er septembre 2026.
...
returns.loadData("sda/data/*.json");
returns.convert({ expenses: "number" }, { strict: false });
returns.summarize({
  columns: "expenses",
  by: "party",
  stats: ["mean", "count"],
...
returns.sort({ mean: "desc" });
await returns.log({ count: 13 });
...
Voici [le lien](https://query1.finance.yahoo.com/v8/finance/chart/AAPL?events=capitalGain%7Cdiv%7Csplit&formatted=true&includeAdjustedClose=true&interval=1d&period1=345479400&period2=1738778777&symbol=AAPL&userYfid=true&lang=en-CA&region=CA) au cas où vous en auriez besoin. Il s'agit d'un point d'accès non documenté : Yahoo peut le modifier ou renvoyer temporairement l'erreur `429 Too Many Requests`. Considérez-le comme un exemple d'inspection du réseau, et non comme une API publique stable.
...
Notez que cet exemple vient du projet [Simulateur boursier 📈](/fr/data-projects/stock-market-simulator). Allez le voir si vous voulez en savoir plus sur l'utilisation d'API non documentées dans vos projets.
...
Voilà qui termine la section des projets de données. Vous apprendrez ensuite comment Git et GitHub peuvent suivre, partager et automatiser votre travail.
