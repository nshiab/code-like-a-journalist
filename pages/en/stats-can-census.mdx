...
To unzip, check your folder outside of VS Code, like in Finder or File Explorer. You might need a software like Winzip or 7-zip. Once unzipped, remove to the three `.zip` files in your project and download folder, and don't forget to empty your trash! 🗑️
...
We can now work on the first million rows in under a second! 😱
...
Let's try to load the names which are in the `2021_92-151_X.csv` file. We can update `crunchData.ts` in the `cache` method. We can keep working in the `cache` method.
...
```ts showLineNumbers filename="crunchData.ts" {35-39}
import { SimpleDB } from "@nshiab/simple-data-analysis";
...
But the projection `proj=lcc` with its `units=m` is problematic. Statistcs Canada uses the Lambert conic conformal projection with coordinates in meters. For many SDA methods to work properly, we need the coordinates with the WGS84 projection using latitude and longitude.

But don't worry, SDA got you covered. You just need to pass the option `{ toWGS84: true }` to convert your geospatial data.

While at it, let's select only the columns we are instered in:
...
And since all of this code in in the `cache method`, it will run just once and let us work quickly on the next steps! As shown above, on the first pass, the code took 4 seconds to run. But on the second, it took only 70 ms.

Structuring, formatting, filtering and cleaning the data is often the longest step in data analysis and visualization. 🫠
