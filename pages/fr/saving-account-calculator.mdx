...
Voici la question que nous voulons attaquer aujourd'hui :
...
Au lieu d’avoir des nombres de `0` à `9`, il serait préférable d’avoir des années réelles.
...
<Callout type="info" emoji="💡">
    Notez que `gainsOrLosses` est créé **à l’intérieur** des accolades de la boucle. Il est **scopé** dans le bloc de code délimité par les `{}`, ce qui signifie qu’il n’existe qu’à l’intérieur de ce bloc. Si vous essayez de l’afficher à la ligne 17, vous obtiendrez une erreur. Le **scoping** est une fonctionnalité importante qui aide à éviter les conflits de variables, mais qui permet également à l’ordinateur de libérer de la mémoire lorsqu’une variable n’est plus nécessaire.
</Callout>
...
Pour randomiser notre taux d’intérêt (qui est actuellement `2`), nous aimerions avoir quelque chose de **moins** ou **plus** que 2, de manière aléatoire. Cette fluctuation pourrait être **positive** ou **négative**.
...
Mettons à jour notre code et réactivons notre tableau. Maintenant, à chaque fois que vous sauvegardez (`CMD` + `S` sur Mac ou `CTRL` + `S` sur PC), vous verrez des résultats différents !
...
```
<Callout type="info" emoji="💡">
    Le `\n` au début des textes aux lignes 35, 37 et 39 est un caractère spécial qui indique un saut de ligne. Il est souvent utilisé dans le terminal pour ajouter des lignes vides et rendre le tout plus lisible.
</Callout>
...
<Callout type="info" emoji="💡">
    Les lignes ajoutées ici (35, 36, 39, 40) utilisent des **template literals**. Lorsque vous écrivez du texte avec des backticks (`` ` ``), vous pouvez insérer des variables ou du code dans le texte en utilisant `${}`. Par exemple, `` `Deux multiplié par deux donne ${2 * 2}` `` créera le texte `"Deux multiplié par deux donne 4"`.
</Callout>
