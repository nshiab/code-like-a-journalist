...
This lesson is just an overview to get you started. We'll explore these data types in more detail as we go. We'll also have fun with data projects down the road.
...
Toggle the terminal (`CMD` + `J` on Mac or `CTRL` + `J` on Windows) and run `deno run --watch --check main.ts`. Deno will clear the terminal and rerun `main.ts` every time you save it (`CMD` + `S` on Mac or `CTRL` + `S` on PC).
...
To create a date object, you need to use the `Date` constructor. Notice the `new` keyword and the parameters passed in the parentheses in the code below. We use `new` because `Date` is a class, which we'll learn about in another lesson.
...
To check if something is `undefined` or `null`, you can use the strict equality operator (`===`), which we'll learn about in another chapter.
...
PS: There's more to explore, including big integers, objects, and functions. We'll cover them when they become relevant in coding projects.
