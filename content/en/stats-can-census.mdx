...
To set up everything we need, let's use [setup-data-project](https://jsr.io/@nshiab/setup-data-project) like in previous lessons.

Create a new folder, open it with VS Code, and run: `deno run -A jsr:@nshiab/setup-data-project`

When prompted, use the space bar to select `simple-data-analysis` and `journalism-extras`, then press `Enter`. Plot is installed automatically with SDA.

Then run `deno task sda` to watch `main.ts` and its dependencies.

![A screenshot of VS Code after running setup-data-project.](/assets/stats-can-census/setup.png)
...
Download the file storing this information from this [direct Statistics Canada link](https://www12.statcan.gc.ca/census-recensement/2021/geo/aip-pia/attribute-attribs/files-fichiers/2021_92-151_X.zip). It is a 9.8 MB zip file named `2021_92-151_X.zip`.
...
The three direct downloads above were verified on September 1, 2026. Their stable filenames are used throughout the lesson.
...
Yes, real-world data projects are always this fun! 😬

### Re-encoding the data

Since re-encoding data is a common task, I created the function [`reencode`](https://jsr.io/@nshiab/journalism-extras/doc/~/reencode) and published it in the [journalism-extras](https://github.com/nshiab/journalism-extras) library selected during setup. So this step will be very easy!
...
To run this script, we can add a `toUTF8` task to the `tasks` already created in our `deno.json`:
...
New files have appeared with `_utf8` in their names! And now your `data` folder weighs... 53 GB. 🤭
...
Let's create a new `crunchData.ts` file to do that. This function will have one parameter, `sdb`, and it will return a `census` table. We will make it asynchronous later when we add the cache.
...
We can finally have a look at the data. With 166 million rows and 23 columns, we have around 3.8 billion data points. 🙃
...
We can also use the `cache` method. Its callback receives the table being cached; queue the synchronous SDA transformations on that `table` parameter. The result is stored in `.sda-cache`, and unchanged code can load it instead of rerunning the computations. SDA 6 also requires temporary tables created inside the callback to be removed with `removeTable()` after use; the later examples do this for the names and boundary tables.
...
On subsequent runs, unchanged computations load from the cache. In the original pre-SDA-6 run on my MacBook Pro, this step was about 10 times faster; use `cacheVerbose` to measure the current gain on your own hardware. 😱
...
And now it works! But this file has a whopping 63 columns. 😳
...
But don't worry, SDA has you covered. You just need to pass the SDA 6 option `{ toEPSG4326: true }` to convert your geospatial data to the right format.
...
And since all of this code is within the `cache` method, it will run just once and let us work quickly on the next steps! In the original pre-SDA-6 run on my computer, the first pass took 1 minute 31 seconds and the cached pass took 97 milliseconds. These are historical measurements, not SDA 6 benchmarks. 😏
...
```
![A screenshot of the cloned table in VS Code.](/assets/stats-can-census/clone.png)
<Callout type="info" emoji="💡">
...
We can now create a function to draw our maps. When we selected SDA in `setup-data-project`, it automatically installed [Plot](https://github.com/observablehq/plot). This very powerful data visualization library works very well with SDA and this is what we are going to use to draw our map.
...
In the original pre-SDA-6 run on my computer, the code crunched all the data and drew the maps in 2 minutes 46 seconds. With the cached analysis, rewriting the 43 maps took 32 seconds. These preserved timings are historical and version-sensitive; use `cacheVerbose` to measure SDA 6 on your own hardware.
