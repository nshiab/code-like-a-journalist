---
title: Écrire et lire des fichiers avec TypeScript
...
Le `./` indique un chemin relatif : il pointe vers le dossier où nous exécutons le script, celui que nous avons créé au début de la leçon.
...
De plus, parce qu'il s'agit d'un fichier JSON, vous pouvez l'importer comme un fichier `.ts` en ajoutant l’attribut d’importation `with { type: "json" }`.
...
Et comme il s'agit de JSON, la vérification des types fonctionne directement, ce qui est formidable.
...
Parfois, vous ne pouvez pas ou ne voulez pas utiliser la syntaxe `import` pour un fichier JSON. Vous pouvez alors le lire avec `Deno.readTextFile()`. Cependant, cette méthode renvoie une chaîne de caractères : le contenu du fichier sous forme de texte.

```ts showLineNumbers filename="main.ts"
const rawData = await Deno.readTextFile("./data.json");
console.log(rawData);
```

![VS Code affichant le contenu du fichier JSON sous forme de texte brut.](/assets/pushing-further/files/json-raw-data.png)

Pour transformer ce texte en tableau ou en objet, utilisez `JSON.parse()`.

```ts showLineNumbers filename="main.ts" {2} /data/3
const rawData = await Deno.readTextFile("./data.json");
const data = JSON.parse(rawData);
console.log(data);
```

![VS Code affichant le tableau d’objets après la conversion du texte JSON.](/assets/pushing-further/files/json-parsed-data.png)
...
Pour identifier une fonction ou une méthode asynchrone, survolez-la. Les fonctions asynchrones retournent une promesse (`Promise`). Pas d'inquiétude: la vérification des types vous alertera si `await` est requis quelque part.
