...
Now, to run and watch this file, we previously used `deno run --watch --check --allow-net=raw.githubusercontent.com main.ts`, but it's a bit annoying to write, and remembering all these options is tiring. The more complex your projects get, the longer these terminal commands can become.
...
The task records the script's exact network permission, so you don't have to remember it and the script still cannot contact other hosts. Run `deno task` without a task name whenever you want to list the available tasks.
...
You now know how to work with TypeScript in complete projects. Next, we'll put these skills to work by acquiring, analyzing, and visualizing data with SDA. 🥳
