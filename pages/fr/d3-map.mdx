...
Now, when working with events or timers, it’s important to remember that you need to **clean them up manually**. For example, what happens if a user clicks the button multiple times? As it stands, it would create multiple `setInterval` calls, and the date paragraph would be updated chaotically!
