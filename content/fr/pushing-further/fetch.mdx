---
title: Récupérer des données avec TypeScript
...
Si vous l'ouvrez, vous remarquerez qu'il s'agit d'une liste d'objets, chaque objet stockant une date sous forme de chaîne de caractères et une température.
...
Ainsi, au lieu d’exécuter votre script avec `deno run --watch --check main.ts` comme indiqué au début de cette leçon, utilisez `deno run --watch --check --allow-net main.ts`.

Puisque nous allons également écrire des données dans un fichier plus tard, autorisons aussi l’écriture.

Cliquez sur votre terminal et appuyez sur `CTRL + C` pour arrêter l'exécution du script (cela fonctionne sur Mac et PC). Ensuite, exécutez : `deno run --watch --check --allow-net --allow-write main.ts`
