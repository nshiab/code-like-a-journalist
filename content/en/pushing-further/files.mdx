...
The `./` makes this a relative path: it points to the folder where we run the script, which is the folder we created during setup.
...
Additionally, because it’s a JSON file, you can import it like a `.ts` file by adding the `with { type: "json" }` import attribute.
...
And because it’s JSON, the type checking works out of the box, which is wonderful.
...
Sometimes, you can’t or don’t want to use the `import` syntax for a JSON file. Instead, you can read it with `Deno.readTextFile()`. However, this gives you a string: the contents of the file as text.
...
![VS Code logging the JSON file contents as raw text.](/assets/pushing-further/files/json-raw-data.png)

To turn that text back into an array or object, use `JSON.parse()`.
...
![VS Code logging the array of objects after parsing the JSON text.](/assets/pushing-further/files/json-parsed-data.png)
