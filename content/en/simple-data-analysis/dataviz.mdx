...
First, we need to store the fire geometries in the same table as the province boundaries. Both datasets use longitude and latitude, so we load them as `EPSG:4326`. Before inserting the rows, we explicitly cast both columns to `GEOMETRY('EPSG:4326')`; this keeps them compatible even if a cached table retains a different CRS type annotation. The two awaited `run()` calls execute those queued type changes before the cross-table insertion is queued.
...
Thank you to [Philippe Rivière](https://github.com/Fil) for the suggestion! 😊
...
These visualizations are saved as images, making them easy to use in articles, reports, and other publications.
...
In the next lesson, we'll complete this section by learning how to use AI with SDA and, just as importantly, how to evaluate its results critically.
