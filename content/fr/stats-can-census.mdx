...
Pour tout configurer, utilisons [setup-data-project](https://jsr.io/@nshiab/setup-data-project) comme dans les leçons précédentes.

Créez un nouveau dossier, ouvrez-le avec VS Code et exécutez : `deno run -A jsr:@nshiab/setup-data-project`

Lorsque le choix des librairies s'affiche, utilisez la barre d'espace pour sélectionner `simple-data-analysis` et `journalism-extras`, puis appuyez sur `Entrée`. Plot est installée automatiquement avec SDA.

Ensuite, exécutez `deno task sda` pour surveiller `main.ts` et ses dépendances.

![Capture d'écran de VS Code après avoir exécuté setup-data-project.](/assets/stats-can-census/setup.png)

## Téléchargement des données

Pour que les noms de fichiers, les colonnes et les valeurs correspondent au code et aux captures d'écran partagées par les deux langues du cours, téléchargez la version anglaise depuis cette [page de Statistique Canada](https://www12.statcan.gc.ca/census-recensement/2021/dp-pd/prof/details/download-telecharger.cfm?Lang=E).
...
Si vous ne le trouvez pas, voici le [lien direct vers le fichier anglais](https://www12.statcan.gc.ca/census-recensement/2021/dp-pd/prof/details/download-telecharger/comp/GetFile.cfm?Lang=E&FILETYPE=CSV&GEONO=006). Cela téléchargera `98-401-X2021006_eng_CSV.zip`, un fichier d'environ 2,25 Go.
...
Téléchargez le fichier contenant ces informations depuis ce [lien direct de Statistique Canada](https://www12.statcan.gc.ca/census-recensement/2021/geo/aip-pia/attribute-attribs/files-fichiers/2021_92-151_X.zip). Il s'agit d'un fichier zip de 9,8 Mo nommé `2021_92-151_X.zip`.
...
Et enfin, comme nous voulons créer une carte, nous avons besoin des frontières géospatiales des aires de diffusion. Vous les trouverez [ici](https://www12.statcan.gc.ca/census-recensement/2021/geo/sip-pis/boundary-limites/index2021-fra.cfm?year=21). Dans le volet *Limites statistiques*, choisissez *Aires de diffusion*, puis le format *Shapefile*. Vous pouvez aussi utiliser le [lien direct vers `lda_000b21a_e.zip`](https://www12.statcan.gc.ca/census-recensement/2021/geo/sip-pis/boundary-limites/files-fichiers/lda_000b21a_e.zip).

Les trois téléchargements directs ci-dessus ont été vérifiés le 1er septembre 2026. Leurs noms de fichiers stables sont utilisés dans toute la leçon.
...
Comme le réencodage de données est une tâche courante, j'ai créé la fonction [`reencode`](https://jsr.io/@nshiab/journalism-extras/doc/~/reencode) et je l'ai publiée dans la librairie [journalism-extras](https://github.com/nshiab/journalism-extras), sélectionnée pendant la configuration. Cette étape sera donc très facile !
...
import { reencode } from "@nshiab/journalism-extras";
...
Pour exécuter ce script, nous pouvons ajouter une tâche `toUTF8` aux tâches déjà créées dans notre `deno.json` :

```ts showLineNumbers filename="deno.json" {5}
{
  "tasks": {
    "sda": "deno run -A --env --watch --check sda/main.ts",
    "all-tests": "deno fmt --check sda && deno lint && deno check sda/main.ts && deno test -A --permit-no-files",
    "clean": "rm -rf .sda-cache .journalism-cache .tmp",
    "toUTF8": "deno run -A sda/helpers/toUTF8.ts"
  },
  "imports": {
    "@nshiab/journalism-extras": "jsr:@nshiab/journalism-extras@^1.0.10",
    "@nshiab/simple-data-analysis": "jsr:@nshiab/simple-data-analysis@^6.0.0",
...
Créons un nouveau fichier `crunchData.ts` pour cela. Cette fonction aura un paramètre `sdb` et retournera une table `census`. Nous la rendrons asynchrone plus tard, lorsque nous ajouterons le cache.
...
export default function crunchData(sdb: SimpleDB) {
  const census = sdb.newTable("census");

  census.loadData("sda/data/98-401-X2021006_eng_CSV/*_utf8.csv", {
    strict: false,
  });
...
Nous pouvons également utiliser la méthode `cache`. Sa fonction de rappel reçoit la table mise en cache : ajoutez les transformations SDA synchrones au paramètre `table`. Le résultat est stocké dans `.sda-cache`, puis le cache peut être relu tant que le code ne change pas. SDA 6 exige aussi que les tables temporaires créées dans la fonction de rappel soient supprimées avec `removeTable()` après leur utilisation; les exemples suivants le font pour les tables de noms et de limites.
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  await census.cache((table) => {
    table.loadData("sda/data/98-401-X2021006_eng_CSV/*_utf8.csv", {
      strict: false,
      limit: 1_000_000,
    });
  });
...
Lors des exécutions suivantes, les calculs inchangés sont chargés depuis le cache. Dans l'exécution originale antérieure à SDA 6 sur mon MacBook Pro, cette étape était environ 10 fois plus rapide; utilisez `cacheVerbose` pour mesurer le gain actuel sur votre ordinateur. 😱
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  await census.cache((table) => {
    ...
    table.renameColumns({ C1_COUNT_TOTAL: "medianIncome" });
  });
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  await census.cache(async (table) => {
    table.loadData("sda/data/98-401-X2021006_eng_CSV/*_utf8.csv", {
      strict: false,
      limit: 1_000_000,
    });
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  await census.cache(async (table) => {
    ...
    const names = sdb.newTable("names");
    names.loadData("sda/data/2021_92-151_X_utf8.csv");
    await names.log();
    await names.removeTable();
  });
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  await census.cache(async (table) => {
    ...
    const names = sdb.newTable("names");
    names.loadData("sda/data/2021_92-151_X_utf8.csv", { strict: false });
    await names.log();
    await names.removeTable();
  });
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  await census.cache(async (table) => {
    ...
    names.selectColumns(["DADGUID_ADIDUGD", "CMANAME_RMRNOM"]);
...
Mais ne vous inquiétez pas, SDA gère cela pour vous. Il vous suffit de passer l'option SDA 6 `{ toEPSG4326: true }` pour convertir vos données géospatiales au bon format.
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  await census.cache(async (table) => {
    ...
    disseminationAreas.selectColumns(["DGUID", "geom"]);
    table.join(disseminationAreas);
    await disseminationAreas.removeTable();
  });
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  await census.cache(async (table) => {
    ...
    table.join(disseminationAreas);
    await disseminationAreas.removeTable();
    table.selectColumns(["medianIncome", "CMA", "geom"]);
    table.removeMissing();
  });
...
Et comme tout ce code est dans la méthode `cache`, il ne s'exécutera qu'une seule fois, ce qui nous permettra de travailler rapidement sur les prochaines étapes ! Dans l'exécution originale antérieure à SDA 6 sur mon ordinateur, le premier passage a pris 1 minute 31 secondes et le passage avec cache, 97 millisecondes. Ce sont des mesures historiques, pas des tests de performance de SDA 6. 😏
...
export default async function crunchData(sdb: SimpleDB) {
  ...
  const medians = census.summarize({
...
export default async function visualizeData(census: SimpleTable) {
  ...
  for (const CMA of allCMAs) {
    const tableCMA = census.clone();
    tableCMA.keepValues({ CMA });
    await tableCMA.log();
  }
}
```
![Capture d'écran de la table clonée dans VS Code.](/assets/stats-can-census/clone.png)
<Callout type="info" emoji="💡">
...
Nous pouvons maintenant créer une fonction pour dessiner nos cartes. Lorsque nous avons sélectionné SDA dans `setup-data-project`, [Plot](https://github.com/observablehq/plot) a été installée automatiquement. Cette puissante librairie de visualisation de données fonctionne très bien avec SDA et c'est ce que nous allons utiliser pour dessiner nos cartes.
...
export default async function visualizeData(census: SimpleTable) {
  ...
  for (const CMA of allCMAs) {
    ...
    const outline = tableCMA.clone();
    await outline.cache((table) => {
      table.aggregateGeo("union");
    });
    tableCMA.insertTables(outline, { unifyColumns: true });
...
Lors de l'exécution originale antérieure à SDA 6 sur mon ordinateur, le code a traité toutes les données et dessiné les cartes en 2 minutes et 46 secondes. Avec l'analyse en cache, la réécriture des 43 cartes a pris 32 secondes. Ces durées conservées sont historiques et sensibles aux versions ; utilisez `cacheVerbose` pour mesurer SDA 6 sur votre propre matériel.
...
Amusez-vous bien avec votre prochain projet de données et n'hésitez pas à [me contacter](/fr/contact) si vous souhaitez partager ce que vous créez avec SDA ou si vous avez des questions ! 😊
