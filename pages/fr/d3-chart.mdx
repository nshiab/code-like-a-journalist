...
- Nous nous assurons de convertir les valeurs `time` en objets `Date` et de les trier. Parfois, elles sont chargées comme des chaînes de caractères.
...
```ts showLineNumbers filename="sda/main.ts" {13-32}
import { SimpleDB } from "@nshiab/simple-data-analysis";
...
```ts showLineNumbers filename="sda/main.ts" {2, 35-71}
import { SimpleDB } from "@nshiab/simple-data-analysis";
...
```ts showLineNumbers filename="sda/main.ts" {72}
import { SimpleDB } from "@nshiab/simple-data-analysis";
import { dot, plot } from "@observablehq/plot";
