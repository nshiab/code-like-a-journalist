...
You'll probably be asked to authenticate. Follow the browser or credential-manager prompt shown by Git. GitHub does not accept your account password for Git operations over HTTPS, so never enter it here. If your setup does not open a browser, GitHub recommends authenticating with [GitHub CLI or Git Credential Manager](https://docs.github.com/en/get-started/git-basics/caching-your-github-credentials-in-git).
...
Now, if you refresh your repository page on GitHub, you'll see that `main` is selected. Open the branch selector or the `Branches` page to explore both branches.
...
<Callout type="info" emoji="💡">
   Have you noticed the `Revert` button? If you merged something too fast and want to revert the changes, this is the button for it! In the previous lesson, we did it manually, but with GitHub it's easier thanks to the PR interface.
</Callout>
...
This all happened in the cloud, on GitHub's platform. To bring the changes to your local repository, first switch to the `main` branch:
- `git switch main`
...
Once a repository is cloned, the next step is usually to install all dependencies with `deno install`. Deno reads `deno.json` and `deno.lock` to install the project's declared, locked dependency versions.
...
For example, several GitHub users have forked the [Simple Data Analysis library](https://github.com/nshiab/simple-data-analysis). I have no idea what they are doing with it and that's okay!
...
For example, let's create a new `password.txt` file in our repo.

```
This is a very important password: potato. Don't share it.
```

![Password in the repository.](/assets/git-and-github/github-basics/password.png)
...
For situations like these, you need to add a `.gitignore` file to your project. If you have one, Git will check it and ignore any folders or files listed in it. They will become greyed out in your project, meaning they won't be tracked, committed, and pushed.

Add `password.txt` to it.

![Adding a gitignore file.](/assets/git-and-github/github-basics/gitignore.png)

Now, the file `password.txt` will be ignored, but you still need to commit and push your new `.gitignore` file:
- `git add -A`
- `git commit -m "Adding .gitignore"`
- `git push`

On your repository page, you can now see your `.gitignore`, but the `password.txt` has been ignored and is not part of your project!

![A gitignore file has been added to the repository.](/assets/git-and-github/github-basics/repo-gitignore.png)

<Callout type="info" emoji="💡">
   If you committed and pushed sensitive information by mistake (like so many of us 😬), [follow these steps](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/removing-sensitive-data-from-a-repository)!
</Callout>
...
Also, when we learn about GitHub Actions to automate tasks like testing, web scraping, and publishing libraries, you'll be amazed by everything this workflow has to offer.
