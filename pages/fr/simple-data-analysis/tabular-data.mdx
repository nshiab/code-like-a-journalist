---
title: How to analyze tabular data with the simple-data-analysis library
...
- `README.md`: A text file with some information about the project.
...
<Callout type="info" emoji="💡">
    If the table layout is displayed weirdly, it's because the width of the table is bigger than the width of your terminal. Right-click on the terminal and look for `Toggle size with content width`. There is also a handy shortcut that I use all the time to do that: `OPTION` + `Z` on Mac and `ALT` + `Z` on PC.
</Callout>
...
Of course, the `"[2020-2049]"` bins are incomplete, but so is the `[1990-2019]` bin for `MONTREAL`. In theory, the 30-year bins should have 10,957 days.
