...
Create a new folder on your computer and run this instead: `deno run -A jsr:@nshiab/setup-data-project`

This tells Deno to run [setup-data-project](https://jsr.io/@nshiab/setup-data-project), which I created to install and configure everything needed when starting a new data analysis. When prompted, use the space bar to select `simple-data-analysis`, then press `Enter`.

After running the command, your terminal will display a description of the created files and installed libraries.

![A screenshot showing VS Code after running setup-data-project.](/assets/simple-data-analysis/tabular-data/setup.png)

### Files created

- `deno.json`: If you click on it, you'll see the `sda`, `all-tests`, and `clean` tasks, along with the installed libraries. We will use `sda` to watch, check, and run `sda/main.ts`.
...
- `.env`: A file for environment variables, such as API keys. It is ignored by Git.
...
- `sda/tests`: An empty folder where you can put tests.
- `docs`: Documentation for the libraries selected during setup.
- `README.md`: A text file containing some information about the project. You can modify it as you wish.
- `AGENTS.md`: Instructions that help coding agents work with the project and its installed libraries.

### Libraries installed

- [`simple-data-analysis`](https://github.com/nshiab/simple-data-analysis): The library we will use to process the data.
- [`@observablehq/plot`](https://github.com/observablehq/plot): A library created by Mike Bostock (creator of d3.js) that we will use to create charts and maps. It is installed automatically with SDA.
...
When you use SDA, you create an **in-memory database**. This is what happens on line 3 with `const sdb = new SimpleDB()`. When we are done with our computations, we close the database on line 7 with `await sdb.close()`. Closing the database also executes any transformations that are still queued and cleans up temporary and cached files.
...
SDA 6 has two kinds of methods:

- **Transformations and loaders** such as `loadData()`, `join()`, `filter()`, and `summarize()` are synchronous. They queue work and immediately return the table, so you can chain them with `.`.
- **Observers and external operations** such as `log()`, `getData()`, `writeData()`, `writeChart()`, and `writeMap()` are asynchronous. When you `await` one, SDA executes the queued work in program order before producing the output.

The same rule applies to AI transformations: `aiRowByRow()` queues the requests, and a later observer actually runs them. If you want to execute a pipeline that ends with transformations without closing the database yet, use `await table.run()` or `await sdb.run()`. At the end of the script, `await sdb.close()` executes any work still queued, saves the necessary metadata, and cleans up the database resources.
...
So, when we write `temperature.join(cities)`, we ask SDA to find a common column (which is `id`) in `temperature` and `cities` and to add the `cities` values with the same `id` to the corresponding `temperature` rows.

Note that I commented out the previous `log` calls on lines 9 and 15 to avoid clogging the terminal.
...
Pass `.cache()` a callback that receives the table being cached. Inside it, queue the transformations on that callback parameter. The first time the callback runs, SDA performs the work and stores the result in `.sda-cache`. Later runs reuse that result until the callback, the table's earlier operations, or a table read by the callback changes. Other non-table values that affect the result can be listed in the `inputs` option.
...
<Callout type="info" emoji="💡">
    Always call `await sdb.close()` at the end of your script. It flushes pending work, releases the database, and removes unused cached data. If you want to clear every cache entry manually, run `deno task clean`.
</Callout>
...
Visualizations in the terminal are quite limited. If you want to create fancier charts, you can use [Observable Plot](https://github.com/observablehq/plot), which was installed when setting up with [setup-data-project](https://jsr.io/@nshiab/setup-data-project).
