...
So instead of running your script with `deno run --watch --check main.ts` as instructed at the beginning of this lesson, use `deno run --watch --check --allow-net main.ts`.

Since we'll also write data to a file later, let's allow writing too.

Click on your terminal and press `CTRL + C` to stop the script from running (this works on both Mac and PC). Then run: `deno run --watch --check --allow-net --allow-write main.ts`
