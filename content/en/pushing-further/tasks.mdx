...
Now, to run and watch this file, we previously used `deno run --watch --check --allow-net main.ts`, but it's a bit annoying to write, and remembering all these options is tiring. The more complex your projects get, the longer these terminal commands can become.
...
This is much easier to type! Also, if you want to add more permissions, such as reading or writing files, just update the command in `deno.json`. You’ll still run the task with the same command: `deno task main`.
...
Tasks are useful in all kinds of projects, whether you’re running a script, scraping the web, analyzing data, or building and publishing a website. They make these commands convenient to run, and we’ll use them throughout the upcoming lessons and projects.

Congratulations, you’ve reached the end of Chapter 2! Read the [chapter conclusion](/pushing-further/conclusion) for project ideas and other ways to put everything you’ve learned in this chapter into practice. 🥳
