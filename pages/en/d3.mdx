...
If you remember the previous lessons, `writeData` creates an array of objects. This exactly what D3 needs. 😉
...
There is a few visual bugs that we can fix right away in `src/helpers/drawChart.ts`:
...
- We can tweak `margins` and the labels positions to avoid cutting off our new text.
...
```
![A scatter plot being redrawn with different variables](/assets/d3/no-animations.gif)

This is cool but it's not animated. The chart just get being redrawned without any transitions. Let rework `src/helpers/drawChat.ts` to create a smooth transition for the circles.

Creating animations with D3 is very easy. You first need to select the elements you want, call `.transition()` and then chain the attribute you want to change.

In our case, we first need to check if we need to draw or to animate. To do that, we check if there is anything already in our `svg` (line 23). If there is nothing, it means we need to draw (lines 53-60). Otherwise, we want to animate the elements already there (lines 62-65).

To animate, we select all the circles, call `.transition()` to tell D3 to interpolate between the current properties of the circles and the new ones right after. Here, we just update `cx` and `cy` with the new domain and range of `xScale` and `yScale`.
...
But it you test the animation right now, it's not very interesting. It's too fast and all the dots are moving at the same time. We can fancy that up by passing a `duration`, an `ease` and a `delay`.

![An basic animation of a scatter plot.](/assets/d3/basic-animation-scatter.gif)

For the duration, I picked 1000 miliseconds. For the ease, I like the `easeCubicInOut` but you have a [lot of options](https://observablehq.com/@d3/easing-animations) with D3. And for the delay, I am just using the duration multiplied by a random number between 0 and 1. So each dot will have a different delay, between 0 and the chosen duration.
...
We could also animate axis and labels.

For the x axis, since it's updating from dates to numbers and vice versa, there is no nice way of switching values like we do with the y axis. So instead, I chained transitions to make it fade, update, and then come back. It's the same trick for the text labels.

Also, note that we don't need to remove the axis or the labels anymore, since we are updating them now.

```ts showLineNumbers filename="src/helpers/drawChart.ts" {73, 76, 82-94, 96, 102-106, 114, 130-154}
import type { earthquake, variable } from "$lib";
import {
  axisBottom,
  axisLeft,
...
Sooo smoooooth! And beautiful! We could keep iterating on this chart to improve even more but I think we've done already a lot! Congratulations! You now know how to create an animate D3 charts! You can be proud!

![An animated scatter plot.](/assets/d3/final-animation-scatter.gif)
