...
- First, you need a variable to store a number. By convention, we call it `i`, but you can name it anything you want. This variable represents the starting number to iterate. Below, it's `let i = 0`.
- Second, you need a condition to stop the loop. This is usually done using comparison operators like `<` or `<=` with the variable. Below, it's `i < 1_000_000`.
- Lastly, you need to specify how to increment the variable. Below, we use `i++`, which means increment by 1 on each step.
