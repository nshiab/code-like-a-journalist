...
<Callout type="info" emoji="💡">
    In VS Code, files are opened as tabs. By default, only one tab is shown. If you want to see multiple tabs side by side, right-click on a tab and then click `Split up`, `Split down`, `Split left`, or `Split right`. In the screenshot below, I chose `Split right`.
</Callout>
