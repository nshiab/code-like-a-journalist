...
```ts showLineNumbers filename="main.ts" {22-24}
import { SimpleDB } from "@nshiab/simple-data-analysis";
...
Let's take of this right away, but using the method `.removeMissing()`. It removes any rows with a `null` value. You can also specify specific columns if you want, but it's not necessary here.

```ts showLineNumbers filename="main.ts" {22}
import { SimpleDB } from "@nshiab/simple-data-analysis";
