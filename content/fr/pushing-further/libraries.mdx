---
title: Utiliser des bibliothèques TypeScript
...
Dans les prochaines leçons, nous utiliserons des bibliothèques en code ouvert pour analyser d'énormes ensembles de données et créer de superbes visualisations.

Une note sur le vocabulaire : les *bibliothèques* sont aussi souvent appelées *libraries*, *packages* ou *modules*.
...
- La liste des responsables inclut **Mike Bostock** et **Philippe Rivière**, deux experts bien connus en visualisation de données.
...
Deno crée ou met aussi à jour `deno.lock`, qui contient des informations supplémentaires sur la bibliothèque. Gardez ce fichier, mais ne vous en préoccupez pas : Deno le gère automatiquement.
...
Copiez-collez le code ci-dessous dans `main.ts`, puis exécutez la commande suivante : `deno run --watch --check --allow-net main.ts`

N'oubliez pas la permission réseau, sinon votre code ne sera pas autorisé à récupérer des données.
...
Parmi les résultats, vous verrez une bibliothèque commençant par `@std`. Cela signifie qu’elle fait partie de la **bibliothèque standard** et qu’elle est maintenue par l’équipe de Deno, qui fournit des bibliothèques de haute qualité et bien testées pour des cas d’usage courants.
...
<Callout type="info" emoji="💡">
    Tout comme les modules NPM, les modules JSR sont mis en cache. Si vous devez vider votre cache, exécutez `deno clean`. Pour installer toutes les bibliothèques listées dans `deno.json` et les vérifier avec `deno.lock`, exécutez `deno install`.
</Callout>
