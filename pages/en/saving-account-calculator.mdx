...
<Callout type="info" emoji="💡">
    Note that `gainsOrLosses` is created **inside** the loop curly brackets. It's **scoped** inside the code block delimited by the `{}`, which means it only exists within it. If you try to log it on line 17, you'll get an error. **Scoping** is an important feature that helps avoid variable conflicts, but it also allows the computer to free up memory when something is no longer needed.
</Callout>
...
Let's update our code and bring back our table. Now, every time you save (`CMD` + `S` on Mac or `CTRL` + `S` on PC), you'll see different results!
...
Using it, we can check whether the final balance is greater than, smaller than, or equal to the initial amount and log the appropriate answer.
...
```
<Callout type="info" emoji="💡">
    The `\n` at the beginning of the strings on lines 35, 37, and 39 is a special character that indicates a line break. It's often used in the terminal to add empty lines and make the output more readable.
</Callout>

It would be great to add more details, like the gains or losses made, both in absolute numbers and as a percentage.

```ts showLineNumbers filename="main.ts" {28-32, 36-40, 43-45}
const initialAmount = 5000;
const interestRate = 2;
const interestRateRange = 20;
const startingYear = 2025;
...
if (finalGainsOrLosses > 0) {
  ...
  console.log(`Final amount: $${lastResult.balance}`);
  console.log(
  ...
  console.log(`Initial amount: $${initialAmount}`);
  console.log(`Final amount: $${lastResult.balance}`);
  console.log(`Losses: $${finalGainsOrLosses} (${perc}%)\n`);
} else {
  console.log("\nYou've neither lost nor gained money!\n");
}
...
![A screenshot showing VS Code logging the results of a saving account calculator.](/assets/saving-account-calculator/results.png)
