...
To allow the script to write a file, you need to explicitly grant permission by adding the `--allow-write` flag to the terminal command. Permissions can be scoped: `--allow-write=.` limits writes to the current project folder instead of the entire computer.
...
To allow the script to read a file, you need to explicitly grant permission by adding the `--allow-read` flag to the terminal command. We can again limit access to the current project folder.
...
## Reading and writing in the project folder 👮

While Deno's explicit permissions are a great security feature, typing several flags can become repetitive during development.

Since we will read and write files in this lesson, we can grant both permissions and limit them to the current project folder.

Click on your terminal, press `CTRL + C` to stop it, and run: `deno run --watch --check --allow-read=. --allow-write=. main.ts`

Now your script can read and write files inside this project without accessing files elsewhere. We will use these scoped permissions for the rest of this lesson.

You may see examples elsewhere using `-A`, the short form of `--allow-all`. It disables Deno's permission sandbox entirely, so it is safer to grant only the access your script needs.
