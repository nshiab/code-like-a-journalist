...
The `.bins()` method needs three arguments:
- The column storing the values
- The interval for the bins
- The name of the new column storing the bins

To check that everything worked as expected, we can use `await temperatures.logUniques("bin")` to display all the unique values in the new `bin` column.
...
![A screenshot showing the bins and logUniques methods.](/assets/simple-data-analysis/tabular-data/bins.png)

The unique values in the `bin` column are looking good. Of course, the last bin, from 2020 to 2049, will be incomplete, but we’ll deal with that later.
