...
For each metropolitan area, we are going to create a map showing the areas with a lower or greater income. The Montréal map below is an example of the final output we'll get together.

![A map of Montreal.](/assets/stats-can-census/map-Montréal.png)

In this real use case project, I am going to show you advanced techniques that I use to crunch big datasets. The 2021 Census data is around 30 GB of data, but you will actually need around 70 GB of free space on your hard drive for this project. Time to do some cleaning! 🧹
...
Now, move all of this inside the `data` folder in your project, and unzip everything except the geospatial boundaries in the file `lda_000b21a_e.zip`! It's unzipped in my screenshots, but it was a mistake 🥲.

To unzip, check your folder outside of VS Code, like in Finder or File Explorer. You might need a software like Winzip or 7-zip. Once unzipped, remove to the two original `.zip` files from your project and download folder, and don't forget to empty your trash! 🗑️ Disk storage is precious when you work with big datasets.
...
I asked Statistics Canada about that and they told me they use the [`Windows-1252`](https://en.wikipedia.org/wiki/Windows-1252) encoding. This means our first step is to re-encode the data...

Yes, real worl data project are always this fun! 😬

### Re-encoding the data

Since re-encoding data often happens, I created the function [`reencode`](https://jsr.io/@nshiab/journalism/doc/~/reencode) and published it in the *[journalism](https://github.com/nshiab/journalism)* library. When you setup your project with `setup-sda`, *journalism* is automatically installed. So this step will be very easy!

Let's create a new file `toUTF8.ts` in `helpers` with the code below. Since we need to reecode the data just once, we are not exporting a function. It's just a regular script.
...
We finally can have a look at the data. With 166 million rows and 23 columns, we have around 3.8 billion data points. 🙃
...
And now it works! But this file has a whooping 63 columns. 😳

### Filtering

If you read [the documentation](https://www150.statcan.gc.ca/n1/pub/92-151-g/2021001/tbl/tbl4_1-eng.htm) (and know your Census pretty well 🥸), you'll realize that you only need two columns, after filtering `CMATYPE_RMRGENRE` for the `B` type to keep only metropolitan areas.
...
Since we want to make a map, we need the dissemination areas boundaries. We downloaded them earlier as the zipped file `lda_000b21a_e.zip` (and I told you to not unzip them 😬).
...
Instead of 197 MB, our geospatial data now weights just 27 MB, which will help speed up our computations and maps drawing!
...
To load the geometries, we can use the `loadGeoData` method. Don't forget to modify the file extension of your simplified Shapefile to `.shp.zip`. The `shp` is important for SDA. Without that, it doesn't know it's a shapefile and can't load it correctly.
...
But don't worry, SDA got you covered. You just need to pass the option `{ toWGS84: true }` to convert your geospatial data in the right format.
...
And let's join the `disseminationAreas` table to the `census` table! Since we have a `DGUID` in each table, SDA will use it to match the census data with the right boundaries.

```ts showLineNumbers filename="crunchData.ts" {40-41}
import { SimpleDB } from "@nshiab/simple-data-analysis";
...
We can remove the limit and select just the three columns that we will use going forward. We can also remove rows with missing values.
...
And since all of this code is within the `cache` method, it will run just once and let us work quickly on the next steps! As shown above, on the first pass, the code took 1 min 31 seconds to run on my computer. But on the second, it took only 97 ms. 😏
...
Always take the time to read the data documentation. It will sometimes feel like a waste of time at the beginning, but it will actually save you a lot of time down the road. I've been there. Trust me. 🫣
...
So we need to find the median household total income for each metropolitan area. This is easy to with the method `summarize`.
...
We now have the answer to our question: the variation from the median in each metropolitan area.
...
As you see in the screenshhot below the new code, when you don't specify a name for tables, SDA automatically name them `table0`, `table1`, etc. Here, we can see that the last table is `table42`, meaning we are looping over 43 metropolitan areas.

To know the number of metropolitan areas, you could also check the `.length` of `allCMAs`, since `getUniques` return all unique values in a column as an array.
...
We can now create a function to draw our maps. When we setup everything with `setup-sda`, we automatically installed [Plot](https://github.com/observablehq/plot). This very powerful datavisualization library works very well with SDA and this is what we are going to use to draw our map.
...
In the loop, we can pass it to the method `writeMap`, which will pass the data from the table as GeoJSON data to our drawing function. We also need to specify where we want the map to be saved and we use the metropolitan area name to create unique files in the `output` folder.

Since we are still in our exploration phase to make the maps, let's break the loop to work just on the first metropolitan area at first. We will be able to iterate faster to improve our dataviz.
...
One last thing that I like to do in my maps usually is to add an outline.
...
We can then insert this newly created geometry to the `tableCMA`.

In the `drawMap` function, we can add a new `geo` mark to draw the outline. But we need to filter on the `varPerc` to make sure that we only draw the outline and not all the disseminationAreas again.
...
The difference is subtle, but I think it important to see the holes in the metropolitan area geometry.

Now that we are happy with our map look, it's time to remove the `break` statement on line 57! Let's create 43 maps!

If you are done running your code and inspect the maps, you'll notice a few problems.

![A screenshot of problematic maps.](/assets/stats-can-census/maps-problems.png)

First, you can see that some folders have been created for some metropolitan areas. It's because some names have `/` in them! We need to replace this character by something else before passing it to `writeMap`.
...
We can also see that the outline for some cities is not right and the map for Montréal is just one gigantic polygon. This usually happens when some geometries are invalid. Simplification often creates invalid geometries. Luckily, we can call the `fixGeo` method to the rescue!
...
Amazing! It runs smoothly and all of our problems dissapeared. 😁

On my computer, our code was able to crunch all of the data and draw all of the maps in 2 minutes 46 seconds. But thanks to our clever caching, if I need to tweak the look of the maps, it's taking just 32 seconds to rewrite all of the 43 maps.

By the way, here, we are saving our maps as images, but if you want vectors to edit them in Illustrator or else, just replace `.png` by `.svg` in `writeMap`.

## Conclusion

What a journey! Crunching big datasets with multiple tables storing tabular and geospatial data is not an easy taks.

But I hope this real life example showed you how you can cut through GB of data like if it was butter with SDA. 🧈

Have fun with your next data project and feel free to [reach out](/contact) if you want to share what you are makin with SDA or if you have any questions! 😊
