...
- An empty `deno.json` file for Deno's project configuration

Open the VS Code Command Palette and run `Deno: Initialize Workspace Configuration` to enable the extension for this folder. Then run `deno run --watch --check main.ts`. This first script does not need any special permissions.
