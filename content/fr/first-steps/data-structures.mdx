...
<Callout type="info" emoji="💡">
  Si la clé `firstName` était écrite avec un espace, comme `"first name"`, vous devriez écrire `student["first name"]` pour accéder à sa valeur. C'est pourquoi la plupart des gens préfèrent utiliser des clés en camel case : c'est plus rapide et plus facile à écrire.
</Callout>
...
Si vous survolez la variable `names`, vous verrez que son type est `string[]`, ce qui signifie qu’il s’agit d’une liste composée exclusivement de chaînes de caractères. Il est généralement préférable de créer des listes contenant un seul type de données (ou un type et `null`, comme nous le verrons plus tard). Cela permet de garder les données organisées et de traiter tous les éléments de la liste de la même façon.
...
Les listes conservent l'ordre de leurs éléments. Étant donné que les listes sont ordonnées, nous pouvons récupérer un élément spécifique en utilisant un **index**. Les index commencent à `0`.
...
La méthode `.sort()` trie les listes contenant des chaînes de caractères par ordre croissant par défaut.
...
Passons maintenant aux conditions et aux boucles.
