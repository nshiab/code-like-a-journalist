...
Before starting, complete **TypeScript fundamentals**, **Working with TypeScript**, and **Data analysis with SDA**. We’ll focus on the HTML structures needed for data extraction as we encounter them.

<NoticeIntro />

## Setup

To set up, use [setup-data-project](https://github.com/nshiab/setup-data-project), as we did in previous lessons.

Create a new folder, open it with VS Code, and run `deno run -A jsr:@nshiab/setup-data-project`.

When prompted, use the space bar to select `simple-data-analysis`, `journalism-web-scraping`, and `journalism-extras`, then press `Enter`.

We will also use Playwright and a Deno file-system helper later in the lesson. Install them by running `deno add npm:playwright-chromium jsr:@std/fs`. Then allow Playwright's installation script to download Chromium by running `deno install --allow-scripts=npm:playwright-chromium --node-modules-dir=auto`.
...
For example, on [this pinned revision of a Wikipedia page](https://en.wikipedia.org/w/index.php?title=Medieval_demography&oldid=1372349632#Demographic_tables_of_Europe's_population) about medieval demography in Europe, you can see multiple tables. Pinning the revision keeps the lesson reproducible even when Wikipedia changes.
...
Since these HTML structures are always the same, I published a function to extract data baked into tables like this in the [journalism-web-scraping](https://github.com/nshiab/journalism-web-scraping) library selected during setup.

### With an `index`

You can pass `getHtmlTable` the URL you want to scrape. Here, we match tables with the `.wikitable` CSS class and use `index: 0` to retrieve the first match. This is more reliable than counting every layout table on the page.
...
<Callout type="info" emoji="💡">
    Caching data is very important. If you don't expect the data to change, you can save it to your computer instead of fetching it again and again. Caching is very easy with the SDA library. The `cache` method creates a `.sda-cache` folder storing data in your project. If you want the data to expire after a moment, check the `ttl` option in the [documentation](https://jsr.io/@nshiab/simple-data-analysis-core/doc/~/SimpleTable.prototype.cache). For more about the SDA library, check out the [`3. Data analysis with SDA 🤓`](/simple-data-analysis/tabular-data) section.
</Callout>
...
For example, here's the [list of all currently sitting Canadian Members of Parliament](https://www.ourcommons.ca/Members/en/search?parliament). This list intentionally changes over time, so you might not have exactly the same results as the screenshots. The page and selectors below were last verified on September 1, 2026.
...
We installed Playwright during setup.

If we inspect the cards, we can see the `a` tag containing the link to the personal MP page. All of these links have the `ce-mip-mp-tile-link` class.
...
- We retrieve all elements with the `ce-mip-mp-tile-link` class (line 9).
- Then we use the `evaluateAll` method, which allows you to run code in the browser. This is very handy when web scraping. Here, we use it to easily extract the `href` attributes to retrieve the URLs (line 10).
...
Let's say we want to retrieve the declared expenses of candidates in the 44th Canadian federal general election. We need to click on multiple options on [the home page](https://www.elections.ca/WPAPPS/WPF/EN/Home/Index). The examples use event value `53`, which identifies that completed election, and the full selector sequence was verified on September 1, 2026.
...
Here's [the link](https://query1.finance.yahoo.com/v8/finance/chart/AAPL?events=capitalGain%7Cdiv%7Csplit&formatted=true&includeAdjustedClose=true&interval=1d&period1=345479400&period2=1738778777&symbol=AAPL&userYfid=true&lang=en-CA&region=CA) in case you need it. This is an undocumented endpoint: Yahoo can change it or temporarily return `429 Too Many Requests`, so treat it as a network-inspection example rather than a stable public API.
...
Note that this example comes from the [Stock market simulator 📈](/en/data-projects/stock-market-simulator) project. Check it out if you want more about using undocumented APIs in your projects.
...
This completes the data projects section. Next, you'll learn how Git and GitHub can track, share, and automate your work.
