...
To set up everything we need, let's use [setup-data-project](https://jsr.io/@nshiab/setup-data-project) as in previous lessons.
...
![A screenshot of VS Code after running setup-data-project.](/assets/simple-data-analysis/sda-and-ai/setup.png)
...
![The Gemini provider settings used in this lesson.](/assets/simple-data-analysis/sda-and-ai/create-key.png)

Once you have copied your new API key, open the `.env` file created by `setup-data-project` and paste it there. Also, add the model we will use, which is `gemini-3.5-flash-lite`.
...
`.env` is a file we usually use for **environment variables**, which are often credentials that should not be shared. `setup-data-project` automatically adds this file to `.gitignore`, so it won't be committed by Git and won't be pushed to GitHub.

![Environment variables in VS Code.](/assets/simple-data-analysis/sda-and-ai/env.png)

The `sda` task created by `setup-data-project` uses Deno's `--env` option, so these variables are loaded automatically whenever you run `deno task sda`.
...
In Google AI Studio, after clicking on `Get API key`, open `API Plan Billing` and select the model we are going to use: `Gemini 3.5 Flash-Lite`.

Check the current request limit shown for your account and model. Quotas can differ by plan and can change over time. The examples below use `rateLimitPerMinute: 30`; lower that value if AI Studio shows a smaller limit for your account.
...
![A reminder to verify the current Google AI Studio quota.](/assets/simple-data-analysis/sda-and-ai/plan-billing.png)

## The data

The goal of the project is to categorize claim expenses. We are going to use the CSV sample data from Ben's tutorial. [It's hosted on GitHub](https://raw.githubusercontent.com/palewire/first-llm-classifier/4d917badc52504be8f850ac6b835ca02607ecf8f/_notebooks/sample.csv) and contains 250 rows.
...
To retrieve the data, we create a new table `data` and cache the results of `loadData`, which fetches the data from GitHub. Since the data won't change, caching it makes sense. It will be stored locally in the `.sda-cache` folder. When you set everything up with `setup-data-project`, this hidden folder is added to your `.gitignore`.
...
- `generation` selects the provider and its settings. Here, `provider: "gemini"` makes the provider explicit. Values in this object override the matching `.env` values.
...
Like the other SDA transformations, `aiRowByRow()` is synchronous and queues its work. The requests are sent when the awaited `log()` observer executes the pipeline.
...
With one row per request, classifying 250 rows requires 250 requests. The rate limit deliberately spaces those requests out, so this can take several minutes.
...
In the code below, we use the `batchSize` option to send 10 rows at a time. That reduces the job from 250 requests to about 25, though the exact duration still depends on the provider and your quota.
...
Since we have 250 rows of data, batches of 17 produce 15 requests. Setting `concurrency: 15` allows those requests to be in flight together, while `rateLimitPerMinute` still controls how quickly SDA starts them. The exact duration depends on the model, current service load, and your account's quota.
...
There is one last optimization to understand: **AI request caching**.

SDA 6 caches AI responses in local files by default. If we rerun our code and the provider, model, prompt, and data have not changed, SDA can use the stored response instead of contacting the model again.

We set `generation.cache` to `true` below to make that default explicit. Set it to `false` when you deliberately need a fresh response on every run.
...
If you run this code, you'll notice a new hidden folder `.journalism-cache`. The files in it are JSON files storing the AI responses. It works like the previous `.sda-cache` folder but is created by the function [`askAI`](https://jsr.io/@nshiab/journalism-ai/doc/~/askAI) from my [`journalism-ai`](https://jsr.io/@nshiab/journalism-ai) library, which the `aiRowByRow` method uses to interact with the AI model.
...
If you have sensitive data that you don't want to send to Google or another AI provider, you can also use [Ollama](https://ollama.com/). Start Ollama, change the provider and model in `.env`, and remove `AI_KEY`:
...
The `generation` object in this lesson explicitly overrides the environment provider. Replace it with `generation: { provider: "ollama", cache: true }` in each `aiRowByRow()` call when using Ollama.
...
So, if you try them in one of your projects, [let me know](/en/contact)!
...
You have now completed the data analysis section. Next, you'll bring these skills together in a series of guided data projects.
