...
In later lessons, we will use open-source libraries to analyze huge datasets and create stunning visualizations.

A note about vocabulary: *libraries* are also often called *packages* or *modules*.
...
Deno also creates or updates `deno.lock`, which contains more information about the library. Keep this file around, but don’t worry about it: Deno manages it automatically.
...
Copy and paste the code below into `main.ts`, then run: `deno run --watch --check --allow-net main.ts`

Don't forget the network permission, or your code won’t be allowed to fetch data.
...
<Callout type="info" emoji="💡">
    Just like NPM modules, JSR modules are cached. If you ever need to clear your cache, run `deno clean`. To install all libraries listed in `deno.json` and verify them against `deno.lock`, run `deno install`.
</Callout>
