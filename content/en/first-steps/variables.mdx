...
The Deno extension is enabled separately for each VS Code workspace. Open the Command Palette (`CMD` + `SHIFT` + `P` on Mac or `CTRL` + `SHIFT` + `P` on PC), then run `Deno: Initialize Workspace Configuration`. This creates `.vscode/settings.json` with `"deno.enable": true`. Repeat this step whenever you start a project in a new folder.
