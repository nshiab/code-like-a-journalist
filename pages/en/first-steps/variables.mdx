...
description: Learn how to use variables, including the difference between `const` and `let`, and the importance of data types.
---
...
Now, let's overwrite `c` with text on line `6`. In TypeScript, text is called a `string` because it's a string of characters. For your computer to recognize it as text, it needs to be wrapped in quotes.
...
We have an error saying that we are trying to put text in `c` while `c` is supposed to be a number! And if we think about it, it makes sense.
...
At this point, you might think, *Why is this so complicated? Let me code whatever I want, stupid computer! I know better than you!*
