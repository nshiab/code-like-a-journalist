...
Create a new folder, open it in VS Code, and run `deno -A jsr:@nshiab/setup-sda --svelte --course`. It will set up everything for you.
...
You can also check the `setup-sda` example. In a new folder, run `deno -A jsr:@nshiab/setup-sda --svelte --example --course` and then run `deno task dev`. You'll see a full example in your browser that uses all of the precoded components you saw in `src/components`, and you can explore the code to learn more.
