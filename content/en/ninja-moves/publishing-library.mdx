...
Before uploading anything, run `deno publish --dry-run`. Deno will type-check the public API and apply JSR's package validations without publishing. Fix any reported errors before continuing.

When the dry run succeeds, run `deno publish`!
...
While we're at it, we can also chain a few commands to make sure our code is well formatted (`deno fmt --check`) and that our types are correct (`deno check`). Using `--check` verifies formatting without changing files during publication.
...
But when you have a lot of libraries to update, run `deno update`. By default, it respects the version ranges in `deno.json`. Use `deno update --latest` only when you intentionally want to move beyond those ranges, including to a new major version.
...
4. When I’m ready to publish a new version (often after a few pull requests), I run one of the `patch`, `minor`, or `major` tasks locally, as defined in the repository's [`deno.json`](https://github.com/nshiab/simple-data-analysis/blob/main/deno.json).

5. These tasks run all checks, then call the [`@nshiab/bump-version`](https://jsr.io/@nshiab/bump-version) tool. It updates `deno.json`, commits and pushes the change, creates a [Git tag](https://git-scm.com/book/en/v2/Git-Basics-Tagging), and pushes the tag to GitHub.

6. When a new tag is pushed to GitHub, two workflows are triggered: [one to publish](https://github.com/nshiab/simple-data-analysis/blob/main/.github/workflows/jsr-publish.yml) the library on JSR, and another to [create a release](https://github.com/nshiab/simple-data-analysis/blob/main/.github/workflows/release.yml) on GitHub with a summary of all the changes from the approved pull requests.
...
With JSR, you can [unlist your library](https://jsr.io/docs/private-registries) so that it doesn’t appear in search results — though it remains publicly accessible. JSR does not currently support private packages.
...
You've completed the core course path. Keep exploring the inspiring people and resources in the next pages, and use what you've learned to create data projects of your own.
