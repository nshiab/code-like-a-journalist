...
![A screenshot of VS Code after running setup-data-project.](/assets/one-billion-row-challenge/setup.png)

## The data

In the original challenge, you have to fork a GitHub repo and run a script to generate a CSV file with 1 billion rows of data. I generated it for you and uploaded a course-owned copy to Google Drive. The file has a stable Drive ID and was verified on September 1, 2026.
...
Congratulations! You just loaded 1,000,000,000 rows of data! In a historical run with SDA 4.0.1 and Deno 2.2.2 on my MacBook Pro M1 with 16 GB of RAM, it took around 26 seconds. Treat this as a reference point rather than a current SDA 6 benchmark. 🥳
...
In that same historical SDA 4.0.1 run, loading and aggregating the one billion values took less than 50 seconds. Hardware, Deno, DuckDB, and SDA versions all affect the result. 🚀
...
In the historical SDA 4.0.1 run, this direct DuckDB query took 16 seconds. It is preserved here to explain the methodology, not as an SDA 6 performance claim. 💥💥💥
...
The chart below is a preserved historical benchmark. I ran the same calculation on the same source data and computer with **simple-data-analysis@4.0.1** (Node.js, Bun, and Deno), **Pandas (Python)**, and the **tidyverse (R)**. Exact versions of the other runtimes and libraries were not recorded, so this is a qualitative historical comparison, not a reproducible SDA 6 benchmark.
