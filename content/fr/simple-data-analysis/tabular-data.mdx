...
La méthode `.bins()` nécessite trois arguments :
- La colonne contenant les valeurs à regrouper
- L’intervalle des groupes
- Le nom de la nouvelle colonne contenant les groupes

Pour vérifier que tout a fonctionné comme prévu, nous pouvons utiliser `await temperatures.logUniques("bin")` pour afficher toutes les valeurs uniques de la nouvelle colonne `bin`.


```ts showLineNumbers filename="main.ts" {27, 30}
import { SimpleDB } from "@nshiab/simple-data-analysis";
...
await temperatures.logUniques("bin");
...
![Capture d’écran montrant les méthodes bins et logUniques en action.](/assets/simple-data-analysis/tabular-data/bins.png)

Les valeurs uniques de la colonne `bin` semblent correctes. Bien sûr, le dernier intervalle, de 2020 à 2049, sera incomplet, mais nous nous en occuperons plus tard.
