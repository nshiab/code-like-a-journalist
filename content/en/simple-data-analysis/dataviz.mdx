...
As in the previous lesson, create a new folder on your computer, open it with VS Code, and run the following command in the terminal: `deno -A jsr:@nshiab/setup-data-project`

When prompted, use the space bar to select `simple-data-analysis` and `journalism-format`, then press `Enter`. Plot is installed automatically with SDA.
...
![A screenshot showing VS Code after running setup-data-project.](/assets/simple-data-analysis/dataviz/setup.png)
...
- We format the y-axis labels with the `formatNumber` function from the [journalism-format](https://github.com/nshiab/journalism-format) library selected during setup.
...
- After calculating the cumulative values, we remove the now-unneeded `sum` column. Then we retrieve the maximum `cumulativeHectares` for each province and store the results in a new table, `maxValuesPerProvince`. We rename its result column to match the one in `fires`, add the final date for our data (October 1st, 2023), and insert those rows into `fires`.
