...
Run `deno run --watch --check main.ts`. This first script does not need any special permissions.
...
Stop the watcher with `CTRL` + `C`, then restart it with the exact network access this project needs: `deno run --watch --check --allow-net=query1.finance.yahoo.com main.ts`.
...
<Callout type="warning" emoji="⚠️">
  Yahoo Finance sometimes blocks requests that do not identify their client. The examples send a minimal `User-Agent` header so Yahoo returns JSON instead of a `Too Many Requests` response.
</Callout>
...
For `period1`, we can set it to `0`, which corresponds to January 1, 1970. Yahoo expects Unix timestamps in seconds. `Date.now()` returns milliseconds, so for `period2` we divide it by 1,000 and round down to fetch the latest available data.
...
<Callout type="info" emoji="💡">
Since I occasionally use Yahoo Finance data in my projects, I published a function that handles all of the above in the [journalism library](https://jsr.io/@nshiab/journalism-finance/doc/~/getYahooFinanceData). We could have used it directly, but I believe there’s a lot of value in understanding how everything works before using functions that feel like magic! 🧙
</Callout>
...
Stop your terminal and install it by running: `deno add jsr:@std/fs`. Deno adds the dependency to `deno.json` and records its exact resolved version and integrity hash in `deno.lock`; commit both files when you use Git.

Then, restart watching `main.ts` with network access to Yahoo and read/write access limited to the cache folder: `deno run --watch --check --allow-net=query1.finance.yahoo.com --allow-read=./data --allow-write=./data main.ts`
...
- Line 7: We use the `symbol` to create a path for the cached data.
- Line 9: We declare a `let` variable for the data.
- Line 10: We check if a file for this company already exists.
  - If it does, we read the data from this file and assign it to the `data` variable (line 12).
- If the file doesn’t exist, the data isn’t cached:
  - We fetch it (lines 15–18), parse it as JSON, and store it in the `data` variable.
  - Then, we cache it by writing a JSON file using the company `path` (line 20).
- Lines 23–24: At this point, the `data` variable contains information that has either been read from a local file or fetched from the API, so we can retrieve the timestamps and values!
