...
For example, the [Simple Data Analysis](https://github.com/nshiab/simple-data-analysis) library version used in this course is `6.0.1`. This numbering convention is called **semantic versioning**, or **SemVer** for short.
...
For example, if you are using `simple-data-analysis@6.0.0` and there is a new version `simple-data-analysis@7.0.0`, check the [release notes](https://github.com/nshiab/simple-data-analysis/releases) to weigh the pros and cons of updating your project with it.
...
For example, if you are using `simple-data-analysis@6.0.0` and there’s a new `simple-data-analysis@6.1.0`, it probably means that there is a new class, new methods, or new functions available. The old ones are still there and should still work as before.
...
Patch versions are usually used to fix bugs and improve things under the hood. They usually don't bring anything new to the table; they just make the library work better.
...
For example, `simple-data-analysis@^6.0.0` means that the project can use compatible versions before `v7.0.0`. So `simple-data-analysis@6.1.0` and `simple-data-analysis@6.1.1` would be acceptable.

### `~` (Tilde)

The `~` is stricter, meaning that the project should stick to a major **and** minor version, but can use any patch version.

For example, `simple-data-analysis@~6.1.0` means that the project can use patch releases before `v6.2.0`, like `simple-data-analysis@6.1.1`.
...
Not every project follows SemVer in exactly the same way. Before updating an important dependency, read that project's versioning policy and release notes instead of relying on the numbers alone.
