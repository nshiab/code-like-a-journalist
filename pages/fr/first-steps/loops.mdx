...
- Tout d'abord, vous avez besoin d'une variable pour stocker un nombre. Par convention, on l'appelle `i`, mais vous pouvez lui donner le nom que vous voulez. Cette variable représente le point de départ pour l'itération. Ci-dessous, c'est `let i = 0`.
- Ensuite, vous avez besoin d'une condition pour arrêter la boucle. Cela se fait généralement en utilisant des opérateurs de comparaison comme `<` ou `<=` avec la variable. Ci-dessous, c'est `i < 1_000_000`.
- Enfin, vous devez spécifier comment incrémenter la variable. Ci-dessous, nous utilisons `i++`, ce qui signifie incrémenter de 1 à chaque étape.
