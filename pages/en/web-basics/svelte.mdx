...
You can create amazing projects just by using HTML, CSS and JavaScript. But the more complex the webpages, the styles, and the user interactions, the more difficult it is to use them.
...
Let's start by crunching our data and prototyping a visualization. This is often the first step in my projects: using SDA to crunch/analyze the data and exploring dataviz options.
...
The first step is to write the data to a file that Svelte will be able to use. We can do that in `crunchData.ts`. To make sure that the page loads as fast as possible, we are going to select just the columns and rows we need for the visualization in our table.
...
Then import this new component in `src/routes/+page.svelte`. To call a component, call its name like your would call an HTML element, with `<` and `/>`.
...
It's now time to make our project **reactive**. Let's say that we want the user to pick a province and we want the fires to be filtered based on its choice.
...
Then let's bind it to the `Select.svelte` component that has been created when set upping with `setup-sda`. The `Select` is a drop down menu that expects three props:
...
- We also retrieve our `provinceFires` and it's type. Note that I am using `unknown` for the properties type to match `drawMap` types. It's a bit loose, but okay.
...
If you want to dig more into Svelte, I recommend you follow their excellent [tutorial](https://svelte.dev/tutorial/svelte/welcome-to-svelte). It's a bit technical and you might not get everything at the first pass, but it's an excellent ressource.

You also check the `setup-sda` example. In a new folder, run `deno -A jsr:@nshiab/setup-sda --svelte --example` and then run `deno task dev`. You'll see a full example in your browser that is using all of the precoded components and you can explore the code to learn more.
